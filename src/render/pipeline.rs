use rendy::hal::{
    pass::Subpass,
    pso::{ AttributeDesc, GraphicsShaderSet, BlendDesc,Primitive,Rect,Viewport,ColorBlendDesc,
           InputAssemblerDesc, Rasterizer, VertexBufferDesc,BasePipeline,DepthTest,GraphicsPipelineDesc,
           DepthStencilDesc,Multisampling,BakedStates,PipelineCreationFlags,Face,VertexInputRate
         },
    Backend,
};
use rendy::factory::{Factory};
use rendy::mesh::VertexFormat;
use crate::render::utils::{vertex_desc};
...
impl<'a, B: Backend> PipelineDescBuilder<'a, B> {
    ...
    pub fn set_vertex_desc(&mut self, desc: &[(VertexFormat, VertexInputRate)]) {
        let (vbos, attrs) = vertex_desc(desc);
...
impl<'a, B: Backend> PipelinesBuilder<'a, B> {
    ...
    pub fn build(self,factory: &Factory<B>,cache: Option<&B::PipelineCache>) -> Result<Vec<B::GraphicsPipeline>, failure::Error> {
        let mut pipelines = unsafe {
            factory.device().create_graphics_pipelines(self.builders.into_iter().map(|b| b.build()), cache)
        };
